      ...
      <div className={style.img_cont}>
        <div className={style.roomtext}>
          ...
          <div className={style.upload_btn}>
            ...
            <input id="image_btn" type="file" accept=".png,.jpg" />
          </div>
        </div>
        {photos.length > 0 &&
          photos.map((photo, index) => (
            <Photoos removeItemOnce={removeItemOnce } index={index} photo={photo}/>
           
          ))}

       
      </div>
      <div  className={photos.length<=2?style.btn_rap:style.btn_rap_scroll}><button>BACK</button><button >CONTINEU</button></div>
    </div>
  );
};
